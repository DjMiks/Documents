    ...
    @IBAction func tapLoginButton(_ sender: Any) {
        ...
        if passInputCount == 0 {
            if passTextField.text!.count >= 4 && passTextField.text != nil {
                ...
                let alert = UIAlertController(title: "Sorry", message: "Password should contain 4 sympols or more", preferredStyle: .alert)
        ...
        if passInputCount == 1 {
            if passTextField.text! == keyChain.get("UserPass") {
                ...
                let alert = UIAlertController(title: "Sorry", message: "Incorrect password, please try again", preferredStyle: .alert)
